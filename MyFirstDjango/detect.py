...
class simulation_opt:
    def __init__(self, weights='static/20220817.pt',
                 ...
                 iou_thres=0.7, device='cpu', view_img=False,
...
class detectapi:
    ...
    def detect(self, source):  # 使用时，调用这个函数
        source0=source.copy()
        height, width, channels = source[0].shape
        if type(source) != list:
        ...
        for img, im0s in dataset:
            ...
            if len(result_txt) > 0:
                ...
                for re in result_txt:
                    ...
                    w = ((r-l)/2.0)/width
                    h = ((b-t)/2.0)/height
                    labels.append(str(str(img_class)+" "+str(x)+" "+str(y)+" "+str(w)+" "+str(h)))
        return result, len(result_txt), labels, label, source0


def save_train_img(img,label):
    i = 0
    i = len(os.listdir("static/train/images"))
    path = "static/train/images/train_" + str(i) + ".jpg"
    txt_path = "static/train/labels/train_" + str(i) + ".txt"
    j = 0
    while os.path.exists(path):
        j+=1
        path="static/train/images/train_" + str(i) + "("+str(j)+").jpg"
        txt_path="static/train/labels/train_" + str(i) + "("+str(j)+").txt"
    img.save(path)
    ls=label.split(",")
...
def save_mark_img(img):
    i = 0
    i=len(os.listdir("static/mark/images"))
    static="static/mark/"
    path = "images/" + str(i) + ".jpg"
    j = 0
    while os.path.exists("static/mark_finish/"+path):
        j += 1
        path = "images/" + str(i) + "(" + str(j) + ").jpg"
